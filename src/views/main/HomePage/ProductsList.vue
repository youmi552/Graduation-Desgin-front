<template>
  <!-- <div class="index-board-list">
    ...
    :key="index"
    :class="['index-board-',{'line-last':index%2!==0}]"
    >
    <div class="index-board-item-inner"  :style=" {background:'url(http://'+item.picture+')' +' no-repeat', backgroundSize: '120px'}" >
        <h2>{{item.goodsName }}</h2>
        <p>{{ item.userName }}</p>
        <div class="index-board-button">
            <li class="button">立即购买</li>
        </div>
    </div>
    </div>
  </div> -->
    ...
    <template #header>
      <div class="card-header" style="height: 20px;">
        <div><span style="float:left">全部商品</span>
        <div style="float: right;width: 200px;">
          <el-input style="width:140px;" v-model="keyword"/>
          <el-button class="button" text @click="search()" style="float:right; width: 50px;">
              <el-tooltip
                 class="box-item"
                 effect="dark"
                 content="搜索"
    ...
    <el-row>
      <div style="width: 100%;" v-show="pageParams.keyword" >
        <span style="font-size:15px;">当前搜索的物品为:{{ pageParams.keyword }}</span>
        <el-button class="button" text @click="clearKeyword()" style=" width: 50px;">
              <el-tooltip
                 class="box-item"
                 effect="dark"
                 content="清空搜索关键字"
  ...
  <el-row class="indexListBoxRow" :span="22" style="margin-left:55px">
    <span style="margin-left:270px" v-show="products==null">暂时还没有任何二手物品</span>
    <el-col
      class="indexList_box"
      v-for="(item, index) in products"
      :key="index"
...
export default {
    ...
    mounted:function(){
      api.getAllProducts(this.pageParams).then(res =>{
